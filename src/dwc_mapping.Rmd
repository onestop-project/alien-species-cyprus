...
The data is maintained in [this Google Spreadsheet](https://docs.google.com/spreadsheets/d/1xN4xmYDG-vdggcT8arDlQrlrgqlxFvNBYS5P1kVhwFY/edit?gid=818849950#gid=818849950).

Read the relevant worksheet (published as csv):

```{r read_source_data}
raw_data <- read_csv("https://docs.google.com/spreadsheets/d/e/2PACX-1vRS3d36CUEVj1G7WQYpyRGHqfcqCeJ9Eg9gR9WQFX_4ihViWq2xCKqEX3lmLSP2gDaNx74_wllBItQ0/pub?gid=818849950&single=true&output=csv", show_col_types = FALSE)
```
...
input_data <-
  ...
  mutate(
    taxon_id = paste(
      "alien_species_cyprus", # e.g. "alien-fishes-checklist"
      "taxon",
      taxon_id_hash,
      sep = ":"
    ))
...
taxon <-
  ...
  mutate(
    ...
    rightsHolder = "the Cyprus Institute", 
    datasetID = "https://doi.org/10.15468/mucze2",
    institutionCode = "CI",
    datasetName = "Checklist of introduced taxa in Cyprus")
```
